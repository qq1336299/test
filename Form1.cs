...
namespace UR_点动控制器
{
    ...
    public partial class Form1 : Form
    {
        ...
        private void Form1_Load(object sender, EventArgs e)
        {
            ...
            //禁用按钮
            confirm_task.Enabled = false;
            cancel_task.Enabled = false;
            cancelTask.Enabled = false;
        ...
        public void ReceiveMessage(IAsyncResult ar)
        {
            ...
            try
            {
                ...
                //var message=@"00##1##on##I:\\guozi\\URController V1.7\\Resources\\up.jpg##222";
                



                //处理得到的字符串
                string[] from_guozi = Regex.Split(message, "##", RegexOptions.IgnoreCase);
                //判断是点信息还是开关状态
                if (from_guozi[0] == "00")
                {
                    GlobalData.ID = from_guozi[1];
                    GlobalData.Status = from_guozi[2];
                    GlobalData.Imagepath_before = from_guozi[3].Replace("/", "\\");
                    GlobalData.Point_Name = from_guozi[4];
                    GlobalData.Command = from_guozi[5].Replace("\0", "");
                    string test = GlobalData.Command;
                    Device_ID.Text = GlobalData.ID;
                    Device_Status.Text = GlobalData.Status;
                    Device_Name.Text = GlobalData.Point_Name;

                    confirm_task.Enabled = true;
                    cancel_task.Enabled = true;


                    try {
                        before_pic.Load(GlobalData.Imagepath_before);
                        after_pic.Image = null;
                    ...
                    Task_Result.Text = "执行中";
                   
                  
                  
                  
                }
                else if (from_guozi[0] == "01")
                {
                    try
                    {
                        GlobalData.Imagepath_after = from_guozi[3].Replace("/", "\\");
                        after_pic.Load(GlobalData.Imagepath_after);
                    }
                    catch
                    {
                    ...
                    if (from_guozi[2] == GlobalData.Status)
                    {
                        socket.Send(Encoding.UTF8.GetBytes(GlobalData.ID + "##fail"));
                        Task_Result.Text = "操作失败";
                        System.DateTime currentTime = System.DateTime.Now;
                        string Time_Str = currentTime.ToString("f");
                        Write_Tasklog(GlobalData.Task_Starttime + "*" + GlobalData.Task_name + "*" + GlobalData.Task_Number + "*" + GlobalData.ID + "*" + GlobalData.Point_Name + "*" + Time_Str + "*" + GlobalData.Imagepath_after + "*" + "操作失败"+ "*" + GlobalData.Status.Replace("\0", "") + "*" + from_guozi[2].Replace("\0", ""));
                    }
                    else
                    {
                        GlobalData.Status = from_guozi[2];
                        Device_Status.Text = GlobalData.Status;
                        socket.Send(Encoding.UTF8.GetBytes(GlobalData.ID + "##success"));
                        Task_Result.Text = "操作成功";
                        System.DateTime currentTime = System.DateTime.Now;
                        string Time_Str = currentTime.ToString("f");
                        Write_Tasklog(GlobalData.Task_Starttime + "*" + GlobalData.Task_name + "*" + GlobalData.Task_Number + "*" + GlobalData.ID + "*" + GlobalData.Point_Name + "*" + Time_Str + "*" + GlobalData.Imagepath_after + "*" + "操作成功" + "*" + GlobalData.Status.Replace("\0", "") + "*" + from_guozi[2].Replace("\0", ""));
                    }
                }
                ...
                else if (from_guozi[0] == "02")
                {
                    //任务信息
                    GlobalData.Task_name = from_guozi[1];
                    GlobalData.Task_Number = from_guozi[3];
                    if (from_guozi[4] == "end")
                    {
                        Task_Endtime.Text = from_guozi[2];
                        from_guozi[2] = from_guozi[2].Insert(13, ":");
                        from_guozi[2] = from_guozi[2].Insert(16, ":");
                        GlobalData.Task_Endtime = from_guozi[2];
                        //Write_Tasklog(":"+from_guozi[1] + ":" +from_guozi[2] + ":" + from_guozi[3]+"*" );
                    }
                    else
                    {
                        GlobalData.Task_Starttime = from_guozi[2];
                        Task_Name.Text = from_guozi[1];
                        from_guozi[2] = from_guozi[2].Insert(13, ":");
                        from_guozi[2] = from_guozi[2].Insert(16, ":");
                        Task_Starttime.Text = from_guozi[2];
                        Task_Number.Text = from_guozi[3];
                        Task_Endtime.Text = "任务未完成";
                        //Write_Tasklog(from_guozi[1] + ":" + from_guozi[2] + ":" + from_guozi[3] + ":");
                    }
        ...
        public void CancelOperation()
        {
            ...
            System.DateTime currentTime = System.DateTime.Now;
            string Time_Str = currentTime.ToString("f");
            Write_Tasklog(GlobalData.Task_Starttime + "*" + GlobalData.Task_name + "*" + GlobalData.Task_Number + "*" + GlobalData.ID + "*" + GlobalData.Point_Name + "*" + Time_Str + "*" + GlobalData.Imagepath_after + "*" + "取消操作" + "*" + GlobalData.Status.Replace("\0", "") + "*" + GlobalData.Status.Replace("\0", ""));
        ...
        void UpdateAnglesValue(double[] Angles)
        {
            ...
            if (TotalCount > 2)
            {
                ...
                RobotRunningLabel.Text = "运动";

            }
            else
            {
                CurrentRunningState = false;
                RobotRunningLabel.Text = "静止";
            }
        ...
        {
            //机械臂复位
            //string Resetcommandone = "movel(p[" + (URDateHandle.Positions_X + 0.20).ToString("0.0000") + "," + (URDateHandle.Positions_Y - 0.20).ToString("0.0000") + "," + URDateHandle.Positions_Z.ToString("0.0000") + "," + URDateHandle.Positions_U.ToString("0.0000") + "," + URDateHandle.Positions_V.ToString("0.0000") + "," + URDateHandle.Positions_W.ToString("0.0000") + "], a = " + AccelerationRate.ToString() + ", v = " + SpeedRate.ToString() + ")";
            //URController.Send_command(Resetcommandone);
            //Thread.Sleep(5000);
            //string Resetcommandone = "movel(p[-0.3026,0.3535,0.4264,-1.462,-0.571,0.643], a = 0.15, v = 0.15)";
            //URController.Send_command(Resetcommandone);
            //Thread.Sleep(5000);
            string Resetcommandtwo = "movel(p[-0.3624,0.1818,0.4447,-1.489,-0.628,0.636], a = 0.05, v = 0.05)";
            URController.Send_command(Resetcommandtwo);
            Thread.Sleep(10000);
            string Resetcommandthree = "movel(p[-0.1965,0.0099,0.5552,-0.067,-0.134,0.909], a = 0.1, v = 0.1)";
        ...
        private void Inspection_result_inquiry_Click(object sender, EventArgs e)
        {
            Form3 srch = new Form3();

            //设置子窗体在主窗体中居中显示
            srch.StartPosition = FormStartPosition.CenterScreen;
            // Display the new form.  
            srch.ShowDialog();
        }

        private void RobticArm_operation_Click(object sender, EventArgs e)
        {
            //定义点击“机械臂操作控制”按钮前后的窗体大小
            Size expandtoArmOp = new Size(1080,731);
            Size closeArmOp = new Size(1080, 461);

            if (this.Size == closeArmOp)
            {
                this.Size = expandtoArmOp;
                this.Top = 0;
            }
            else
                this.Size = closeArmOp;
        }

        //private void button3_Click_2(object sender, EventArgs e)
        //{
        //    Commandtransmit.Send_command("picture##");
        //    string picPath = "\\\\GZROBOT_PC\\SavedImages_up";
        //    string[] picInf = Directory.GetFiles(picPath, "*.jpg", SearchOption.AllDirectories);
        //    foreach(string selPicPath in picInf)
        //    {
        //        this.pictureBox2.Load(selPicPath);
        //    }
        //}

        private void LightenOn_Click(object sender, EventArgs e)
        {
            string Lightenoncommand = "set_digital_out(0,True)";
            URController.Send_command(Lightenoncommand);
        }

        private void LightenOff_Click(object sender, EventArgs e)
        {
            string Lightenoffcommand = "set_digital_out(0,False)";
            URController.Send_command(Lightenoffcommand);
        }

        private void confirm_task_Click(object sender, EventArgs e)
        {
            //Commandtransmit.Creat_client("127.0.0.1", 4333);
            cancel_task.Enabled = false;
            cancelTask.Enabled = true;

            Thread Transmit_confirm = new Thread(transmit2_down);
            Transmit_confirm.Start();
        ...
        private void cancel_task_Click(object sender, EventArgs e)
        {
            CancelOperation();
            GlobalData.User_Operating = "none";
            confirm_task.Enabled = false;
            cancel_task.Enabled = false;
            cancelTask.Enabled = false;
        }
        public void transmit2_down()
        {
            confirm_task.Enabled = false;
            GlobalData.back_message = Commandtransmit.Send_command_WithFeedback("confirm##" + GlobalData.ID + "##" + GlobalData.Status + "##" + GlobalData.Command + "##");
            var str = GlobalData.back_message;
            if (str == "cancel")
            {
                MessageBox.Show("中止任务成功");
            }
            socket.Send(Encoding.UTF8.GetBytes(GlobalData.back_message));
            GlobalData.User_Operating = "none";
